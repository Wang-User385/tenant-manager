...
//        this.setSelectStatus(record);
            ...
            if(div){
                ...
                if(editor!=_N && ($le(editor) instanceof A.CheckBox)){
...
//        sf.drawFootBar(name);
        ...
        updateTitleOrder :function(){
            var sf = this;
            $jq(sf.tbody.dom).find('.form_head').each(function(index,td){

                var div = $jq(td).find(".title");

                $jq(div).text($jq(div).data('title')+ '（' + sf.numToChinese(index+1) + ')');
        ...
        createRow : function(record,index){
            ...
            //新增头配置信息
            if(index != 0){
                var headLength = $jq(sf.tbody.dom).find(".form_head").length;
                var str = '<tr _id="'+sf.id+'" _row="'+record.id+'">'+$jq(sf.tbody.dom).find("tr").eq(0).html()+'</tr>';
                var node = $jq(str);
                node.find(".gridbox-button").remove();
                node.appendTo(sf.tbody.dom);
                index = index + headLength;
            }
        ...
        createCloseBtn : function(td,record){
            new Ext.Template('<div class="gridbox-close-button" recordid="{recordid}"><span class="gridbox-close-span">删除</span></div>').append(td,{
                recordid:record.id
            });
        ...
        createCell:function(tr,col,record){
            ...
            if(col){
                ...
                if(col.name&&record.ds.fields[col.name]&&record.ds.fields[col.name].isRequired()){
                    isRequired=true;
                }
                if(isRequired){
                    th.innerHTML = '<div><span style="color: red;">*</span>' + prompt + (IS_EMPTY(prompt) ? _N : sf.labelseparator) + '</div>';
                }else {
        ...
        onClick : function(e,t) {
            var sf = this,
                ...
                ds = sf.dataset,
                record;
            if(target.is('div.gridbox-close-button')||target.is('span.gridbox-close-span')){
...
//            }
        ...
        setEditor: function(name,editor){
            ...
            /*if(div){
             ...
             }else if(!$le(editor) instanceof A.CheckBox){
        ...
        positionEditor:function(){
            ...
            if(ed instanceof A.CheckBox){
                ...
                ed.setWidth(dom.getWidth()-3<22?22:(dom.getWidth()-3));
                ed.move(xy[0],xy[1]);
            }
