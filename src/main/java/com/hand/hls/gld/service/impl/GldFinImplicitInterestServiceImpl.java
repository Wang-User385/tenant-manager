...
public class GldFinImplicitInterestServiceImpl implements IGldFinanceIncomeDayCommonService {
    public static final String SHARE_TYPE = "IMPLICIT_INTEREST";
    @Autowired
    ...
    public void shareLease(IRequest iRequest, List<HlsCusConContract> list, Map params) throws ParseException {
        ...
        for (HlsCusConContract contract : list) {
            ...
            //获取日利率 后续计算
            AmortizationInterest amortizationInterest = utilService.getIntRateDay(contract.getContractId(), SHARE_TYPE);
            //必须有利息才摊销
            if (amortizationInterest.getNetInterestTotal() > 0) {
                //构造AmortizationInterest 方便后续插入业务表
                amortizationInterest = utilService.getAmortizationCashflow(contract.getContractId(), amortizationInterest, SHARE_TYPE);
                //插入 未实现融资收益日表
