...
import com.hand.hls.elecSeal.dto.HttpResponse;
...
public class HttpUtils {
    ...
    public static HttpResponse postFormData(String urlStr, Map<String, byte[]> filePathMap, Map<String, Object> keyValues, Map<String, Object> headers) throws IOException {
        ...
        try (DataOutputStream out = new DataOutputStream(conn.getOutputStream())) {
            ...
            if (filePathMap != null && !filePathMap.isEmpty()) {
                for (Map.Entry<String, byte[]> filePath : filePathMap.entrySet()) {
     ...
     * @throws IOException
     */
    private static void writeFile(String paramName, byte[] fileData, String boundary,
                                  DataOutputStream out) {
        try (BufferedReader fileReader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(fileData)))) {
            /**
             * 写分隔符--${boundary}，并回车换行
             */
             ...
             */
            String contentDispositionStr = String.format("Content-Disposition: form-data; name=\"%s\"; filename=\"%s\"", paramName, fileData) + LINE_END;
