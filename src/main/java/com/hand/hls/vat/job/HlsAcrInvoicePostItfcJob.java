...
import com.hand.hls.vat.dto.AcrInvoiceHdTemp;
 ...
 */
@Component
public class HlsAcrInvoicePostItfcJob extends AbstractJobWithIRequest {
    ...
    public void safeExecuteWithIRequest(JobExecutionContext context, IRequest requestCtx) throws Exception {
        try {
            ...
            acrInvoiceHd.setInvoiceStatus("NEW");
            List<HlsCusAcrInvoiceHd> list = acrInvoiceHdMapper.queryAcrInvoiceHdDetailNew(acrInvoiceHd);

            for (HlsCusAcrInvoiceHd item : list) {
                if (item.getTotalAmount() < 0) {
                    invoiceBaseUtils.reverseInvoiceItfc(requestCtx, item);
                } else {
                    invoiceBaseUtils.createInvoiceItfc(requestCtx, item);
                }
