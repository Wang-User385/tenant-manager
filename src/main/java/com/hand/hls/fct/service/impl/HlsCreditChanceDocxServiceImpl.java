...
import com.hand.hls.fct.service.HlsCreditChanceDocxService;
...
public class HlsCreditChanceDocxServiceImpl implements HlsCreditChanceDocxService {

    private static final String CREATE_DOCX_FAILURE = "立项业务意向书生成失败!";

    private static final String BAK = "_back_";

    private static final String DOCX_FILE_NAME = "业务意向书";
    ...
    public static final String NOT_FOUND_CONTRACT_TEMPLATE  = "找不到对应的立项业务意向书模板";
    public static final String TABLE_NAME = "table_name";
    public static final String CONTRACT_DOCX_DESCRIPTION = "立项业务意向书";
    ...
    public List<FndAttachment> docxCreateMethod(IRequest requestContext, Map<String, Object> params) throws Exception {
        ...
        Long chanceId = Long.parseLong(params.get(CHANCE_ID).toString());
        Long chanceAttachmentId = Long.parseLong(params.get(CHANCE_ATTACHMENT_ID).toString());
        //项目信息
        HlsCusHlsCreditLineChance record = new HlsCusHlsCreditLineChance();
        record.setChanceId(chanceId);
        HlsCusHlsCreditLineChance report = chanceService.selectByPrimaryKey(requestContext, record);
        //合同文本的SourceType
        String SourceType = FILE_DOC_SOURCE_TYPE;
        //模板的ID为对应的SourceKey
            ...
            if (sysFile != null && StringUtils.isNotBlank(sysFile.getFilePath())) {
                ...
                if (file.exists()) {
                    ...
                    if (fileLength > 0) {
                        ...
                        condition.setTablePkValue(chanceAttachmentId.toString());
                        ...
                        if (!list.isEmpty() && list.size() > 0) {
                            ...
                            conDocFileMulti.setTablePkValue(chanceAttachmentId.toString());
...
//                            conDocFile.setSourcePkValue(contractAttachmentId.toString());
                    ...
                    //重新插入合同文本记录--插入生成文件名
                    HlsCreditLineAttach ppa = new HlsCreditLineAttach();
                    ppa.setChanceAttachmentId(chanceAttachmentId);
                    ppa = creditLineAttachService.selectByPrimaryKey(requestContext, ppa);
                    if (ppa.getDocumentName() == null || StringUtils.isBlank(ppa.getDocumentName())) {
                        if (report != null) {
                            ppa.setDocumentName(DOCX_FILE_NAME + report.getCreditLineNumber() + fileType);
                        } else {
                            ppa.setDocumentName(DOCX_FILE_NAME + fileType);
                        }
                        creditLineAttachService.updateByPrimaryKeySelective(requestContext, ppa);
                    }
