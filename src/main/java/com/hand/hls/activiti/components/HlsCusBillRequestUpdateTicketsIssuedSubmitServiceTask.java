...
public class HlsCusBillRequestUpdateTicketsIssuedSubmitServiceTask implements JavaDelegate, IActivitiBean {
    @Autowired
    private hlsBillRequestMapper hlsBillRequestMapper;
    @Autowired
    private IhlsBillRequestService hlsBillRequestService;
    public HlsCusBillRequestUpdateTicketsIssuedSubmitServiceTask() {
    }
    ...
    public void execute(DelegateExecution delegateExecution) {
        ...
        if ("APPROVED".equalsIgnoreCase(result)) {
            hlsBillRequest.setBillStatus("TICKETS_ISSUED");
