...
/**
 * @Description:投资理财申购工作流启动
 * @Author: wty
 * @Date: Created in 10:34 2018/4/26
 */
@Service
@Transactional(rollbackFor = Exception.class)
public class HlsCusInvPurchaseActivitiStartServiceImpl implements IActivitiCommonService {

    private static final String WORK_FLOW_TYPE = "INV_PURCHASE_WFL";
    ...
    private ProcessInstanceCreateRequest getProcessInstanceCreateRequest(IRequest iRequest, HlsCusFinancePurchase purchase) {
        ...
        ReProcdef reProcdefs = reProcdefService.queryReProcdef("INV_PURCHASE_WORK_FLOW", "INVESTMENT");
    ...
    public void cancel(IRequest iRequest, Map params) {
        ...
        if (CollectionUtils.isNotEmpty(attachments)) {
            for (int i = 0; i < attachments.size(); i++) {
                attachments.get(i).setStatus("NEW");
