...
/**
 * Created by zhangdan on 2022/08/31.
 */
@Service
@Transactional(rollbackFor = Exception.class)
public class HlsCusProjectCreditOccupyActivitiStartServiceImpl implements IActivitiCommonService {
    private static final String workFlowType = "QUOTA_OCCUPATION_WFL";
    @Autowired
    ...
    private ProcessInstanceCreateRequest getProcessInstanceCreateRequest(HlsCusPrjProject hlsCusPrjProject, IRequest iRequest) {
        ...
        ReProcdef reProcdefs = reProcdefService.queryReProcdef(workFlowType, workFlowType);
        ...
        restVariable45.setName("meetingFlag");
        restVariable45.setValue(hlsCusPrjProject.getMeetingFlag());
        variables.add(restVariable45);

        //是否授信项目
        RestVariable restVariable46 = new RestVariable();
        restVariable46.setName("creditFlag");
        restVariable46.setValue(hlsCusPrjProject.getCreditFlag());
        variables.add(restVariable46);

        RestVariable restVariable47 = new RestVariable();
        restVariable47.setName("dataType");
        restVariable47.setValue(hlsCusPrjProject.getDataType());
