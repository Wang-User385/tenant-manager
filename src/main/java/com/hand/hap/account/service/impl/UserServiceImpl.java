...
public class UserServiceImpl extends BaseServiceImpl<User> implements IUserService {
    ...
    public final static String UPLOAD_PATH_PREFIX = "/src/main/resources/leaf_static/resources/images/profile_photo/";
    ...
    @Override
    @Transactional(propagation = Propagation.SUPPORTS)
    public User selectByAttribute3(String attribute3) {
        User user = null;
        if (attribute3 !=null) {
            user = userMapper.selectByAttribute3(attribute3);
        }
        return user;
    }
    ...
    @Override
    public List<User> selectUserByEmployeeCode(String employeeCode) {
        return userMapper.selectUserByEmployeeCode(employeeCode);
    }
...
//        criteria.selectExtensionAttribute();
    ...
    public List<User> batchUpdate(IRequest request, List<User> list) {
        ...
        criteria.update(User.FIELD_EMPLOYEE_ID, User.FIELD_EMAIL, User.FIELD_PHONE,
                ...
                User.FIELD_DESCRIPTION,User.FIELD_BP_ID, User.FIELD_BP_CATEGORY, User.FIELD_INTRANET_FLAG);
    ...
    @Override
    public Long selectEmployeeAssginIdByUserId(Long userId) {
        return userMapper.selectEmployeeAssginIdByUserId(userId);
    }

    @Override
    public void reloadUserCacheByUserId(Long userId) {
        User user = userMapper.selectByUserId(String.valueOf(userId));
        if (user != null && StringUtils.isNotEmpty(user.getUserName())) {
            User usertoUpdate = userMapper.selectByUserName(user.getUserName());
            userCache.setValue(user.getUserName(), usertoUpdate);
        }
    }

    @Override
    public List<User> queryUsersForLov(IRequest iRequest, User user, int page, int pagesize) {
        PageHelper.startPage(page, pagesize);
        return userMapper.selectForLov(user);
    }
    ...
    @Override
    public List<User> selectUserByPositionId(Long positionId) {
        return userMapper.selectUserByPositionId(positionId);
    }

    @Override
    public User queryUserByAllocationId(String allocationId) {
        return userMapper.queryUserByAllocationId(allocationId);
    }
     ...
     * @Discription: 更新用户头像
     * @Param: [iRequest, newProfile]
     * @Return: void
     */
    @Override
    public void updateUserProfile(IRequest iRequest, MultipartFile newProfile) {
        ...
        String profilesPath = filePath + UPLOAD_PATH_PREFIX;
        //1、上传文件到指定路径
        if (!newProfile.isEmpty()) {
            ...
            String profilePathAndNameDB = user.getImgPath();
            String newProfileName = profilePathAndNameDB;

            // 若头像名称不存在
            if (profilePathAndNameDB == null || "".equals(profilePathAndNameDB)) {
                //修改文件名 用户id + png 后缀
                newProfileName = profilesPath + user.getUserId() + ".png";
                // 路径存库
                user.setImgPath(newProfileName);
                //更新数据库 头像路径
