...
import com.hand.hap.activiti.listeners.NotificationApproveListener;
import com.hand.hap.activiti.listeners.NotificationTodoListener;
import com.hand.hap.activiti.listeners.WflApprovalRecordListener;
...
public class CustomUserTaskParseHandler extends UserTaskParseHandler implements IActivitiConstants {
    ...
    @Autowired
    private NotificationTodoListener notificationListener;
    @Autowired
    private NotificationApproveListener notificationApproveListener;
    @Autowired
    private WflApprovalRecordListener wflApprovalRecordListener;
    @Autowired
    ...
    protected void executeParse(BpmnParse bpmnParse, UserTask userTask) {
        ...
        completeListener.setInstance(notificationApproveListener);
        userTask.getTaskListeners().add(completeListener);
        /////// multiInstance or approveChain


        //---start for 兴业记录工作流相关信息 及 审批人的mac地址
        ActivitiListener approvalRcordsListener = new ActivitiListener();
        approvalRcordsListener.setEvent(TaskListener.EVENTNAME_COMPLETE);
        approvalRcordsListener.setImplementationType(ImplementationType.IMPLEMENTATION_TYPE_INSTANCE);
        approvalRcordsListener.setInstance(wflApprovalRecordListener);
        userTask.getTaskListeners().add(approvalRcordsListener);
        //---end
            ...
            if (record != null) {
                ...
                if (num != null) {
                    if (StringUtils.isNotEmpty(num.getName())) {
                        condition = condition.replace(APPROVAL_WFL_NUM, num.getName());
                    }
                }
            }
        }
