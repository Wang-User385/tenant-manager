...
public class CompositeLoader {
    ...
    */
    ...
    protected CompositeMap loadByURL_NC(String url) throws IOException, SAXException {
        ...
        try {
            if(StringUtils.startsWith(url, "classpath:")){
                return parse(ResourceUtils.getURL(url).openStream());
            }
    ...
    String getFullPath(String file_name) {
        ...
        if (this.mBaseDir != null) {
            if(full_name.startsWith("/")){
                full_name = this.mBaseDir + full_name.substring(1);
            }else {
    ...
    public CompositeMap loadByFile(String file_name) throws IOException, SAXException {
        String fullName = getFullPath(file_name);

        try {
            if (fullName.startsWith("classpath")) {
                return loadByURL(fullName);
            }
            return loadByFullFilePath(fullName);
	...
	*/
    ...
    private CompositeMap loadFromClassPath_NC(String full_name, String file_ext) throws IOException, SAXException {
        ...
        try {
            ...
            if (need_stream) {
                ...
                if (stream == null) {
                    url = this.mClassLoader.getResource("/");
                    File f = new File(url.getFile()).getParentFile();
                    stream = new FileInputStream(new File(new File(f, "models"), path));
                    if (stream == null) {
    ...
    public void setBaseDir(String baseDir) {
        ...
        if (!f.exists()) {
            throw new IllegalArgumentException("Directory not exists:" + baseDir);
        }
        int len = baseDir.length();
        if (baseDir.charAt(len - 1) != '\\' && baseDir.charAt(len - 1) != '/') {
            this.mBaseDir = baseDir + File.separatorChar;
        } else {
            this.mBaseDir = baseDir;
        }
    }
