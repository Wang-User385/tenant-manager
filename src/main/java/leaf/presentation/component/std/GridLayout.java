 ...
 * GridLayout.
 * 
 * @version $Id: GridLayout.java,v 1.1 2016/09/02 08:21:30 wuhuazhen2689 Exp $
...
//	protected static final String PROPERTITY_WRAPPER_ADJUST = "wrapperadjust";
	...
	protected int getDefaultHeight(BuildSession session){
		return 0;
	}
	...
	protected int getPadding(BuildSession session){
		return GridLayoutConfig.DEFAULT_PADDING;
	}
	...
	private String buildCell(BuildSession session, CompositeMap model, CompositeMap view, CompositeMap field, StringBuilder upper, StringBuilder under) throws Exception{
		...
		if(null!=field){
			...
			int padding = glc.getPadding(model,getPadding(session));
	...
	//					width = Component.class.getDeclaredMethod("getDefaultWidth(BuildSession session)").invoke(kls.newInstance()).toString();
	//				}catch (Exception e) {
	//				}
	//			}
				if(null!=width 
	//					&& Integer.parseInt(width)>0
			...
			if(session.getTheme().toUpperCase().indexOf("HAP") != -1){
				if(!"screenTopToolbar".equalsIgnoreCase(field.getQName().getLocalName())){
					sb.append("' style='padding:3px;");
				}else{
					if("bottom".equalsIgnoreCase(field.getString("type"))){
						session.setBottomToobar(true);
					}
					sb.append("' style='padding:"+padding+"px;");
				}
                session.setScreenTopToolbar(true);
			}else {
				sb.append("' style='padding:"+padding+"px;");
			}
...
//			addInvalidMsg(field, out);
	...
	public String buildRows(BuildSession session, CompositeMap model, CompositeMap view, Iterator it) throws Exception{
		StringBuilder sb = new StringBuilder();
		while(it.hasNext()){
...
//		int height = Integer.valueOf(hstr).intValue();
	...
	protected void buildGridLayoutScript(BuildSession session,String id) throws Exception{
		Writer out = session.getWriter();
		out.write("<script>Leaf.doGridLayoutCustom('"+id+"')</script>");
	}
	...
	public void buildView(BuildSession session, ViewContext view_context) throws IOException, ViewCreationException {
		...
		try {
			...
			buildGridLayoutScript(session,id);
		} catch (Exception e) {
...
//		out.write("<script>");
//		StringBuilder sb = new StringBuilder();
//		sb.append("new Leaf.Box({id:'").append(id).append("',");
//		sb.append("cmps:[");
