...
import leaf.presentation.component.std.config.FieldConfig;
...
/**
 * Box
 * @version $Id: Box.java,v 1.1 2016/09/02 08:21:30 wuhuazhen2689 Exp $
		...
		if(align.equals(labelPosition)){
			...
			if(null != field){
				...
				if(!"".equals(label)) {
					...
					sb.append("width="+labelWidth+"><div style='min-width:56px;text-align:"+labelAlign+"'");
					if(!"".equals(id)){
					...
					if("left".equals(align)){
						/**
						 * author:gaoyang
						 * time:2018-5-22 13:01:56
						 * purpose:必输字段prompt增加红色*
						 */
						sb.append("<span class='prompt-required");
						if(getFieldRequired(session, field, field.getString(ComponentConfig.PROPERTITY_BINDTARGET, ""))) {
							sb.append(" prompt-required-show");
						}
						sb.append("' title='" + field.getString(ComponentConfig.PROPERTITY_NAME, "") + "'>*</span>");
						sb.append(label);
